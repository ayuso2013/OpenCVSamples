			   ...
			   bool addNewOnly)
{
	...
	fabmap->compare(testImageDescs, fabmap->getTrainingImgDescriptors(), matches);
	...
	for (int i;i<=testImageDescs.rows;i++)
		for (int j=0;j<testImageDescs.rows;j++)
			confusion_mat.at<double>(i,j)=0.0;
	...
	if (!addNewOnly) {
		...
		for(int i = 0; i < testImageDescs.rows; i++) {
			...
			for(l = matches.begin(); l != matches.end(); l++) {
		//		writer << l->queryIdx << "," << l->imgIdx << "," << l->likelihood << "," << l->match << std::endl;
				if(l->imgIdx < 0) {
