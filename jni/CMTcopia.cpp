...
bool CMT::initialise(cv::Mat im_gray0, cv::Point2f topleft, cv::Point2f bottomright)
{
	...
	sprintf(s,"paso A %Ld",ticks_ms());
	ALOG(s, " ");

   sprintf(s,"paso C %Ld",ticks_ms());
   	 ALOG(s, " ");
    descriptorMatcher = cv::DescriptorMatcher::create(matcherType.c_str());
    sprintf(s,"paso D %Ld",ticks_ms());
   	 ALOG(s, " ");
    ...
    selectedClasses.reserve(selected_keypoints.size());
    ...
    classesDatabase.reserve(backgroundClasses.size()+selectedClasses.size());
    ...
    squareForm.reserve(selected_keypoints.size());
    angles.reserve(selected_keypoints.size());
    for(unsigned int i = 0; i < selected_keypoints.size(); i++)
    {
        std::vector<float> lineSquare;
        std::vector<float> lineAngle;

        lineSquare.reserve(selected_keypoints.size());
        lineAngle.reserve(selected_keypoints.size());
    ...
    springs.reserve(selected_keypoints.size());
    for(unsigned int i = 0; i < selected_keypoints.size(); i++)
    ...
    activeKeypoints.reserve(selected_keypoints.size());
    for(unsigned int i = 0; i < selected_keypoints.size(); i++)
...
bool CMT::Save(const char *path)
{
	cv::FileStorage store(path, cv::FileStorage::WRITE);
	cv::write(store,"im_prev",im_prev);

	cv::write(store,"detectorType",detectorType);
	cv::write(store,"featuresDatabase",featuresDatabase);

	std::vector<cv::KeyPoint> kp_object;
	std::vector<int> kp_object_int;

	for (int i=0;i<activeKeypoints.size();i++)
	{
		kp_object.push_back(activeKeypoints[i].first);
		kp_object_int.push_back(activeKeypoints[i].second);
	}
	cv::write(store,"activeKeypoints",kp_object);
	cv::write(store,"activeKeypoints_int",kp_object_int);

	cv::write(store,"classesDatabase",classesDatabase);
	cv::write(store,"springs",springs);
	cv::write(store,"selectedFeatures",selectedFeatures);
	cv::write(store,"selectedClasses",selectedClasses);
	cv::write(store,"votes",votes);

	kp_object.clear();
	kp_object_int.clear();
	for (int i=0;i<outliers.size();i++)
		{
			kp_object.push_back(outliers[i].first);
			kp_object_int.push_back(outliers[i].second);
		}
	cv::write(store,"outliers",kp_object);
	cv::write(store,"outliers_int",kp_object_int);

	kp_object.clear();
	kp_object_int.clear();
	for (int i=0;i<trackedKeypoints.size();i++)
		{
			kp_object.push_back(trackedKeypoints[i].first);
			kp_object_int.push_back(trackedKeypoints[i].second);
		}
	cv::write(store,"trackedKeypoints",kp_object);
	cv::write(store,"trackedKeypoints_int",kp_object_int);

	store << "nbInitialKeypoints" << (int)nbInitialKeypoints;

	store << "squareFormCount" << (int)squareForm.size();



	char s[20];

for (int i=0;i<squareForm.size();i++)
{
	sprintf(s,"squareForm%d",i);
	cv::write(store,s,squareForm[i]);
}
	store << "anglesCount" << (int)angles.size();

for (int i=0;i<angles.size();i++)
{
	sprintf(s,"angles%d",i);
	cv::write(store,s,angles[i]);
}
	//std::vector<cv::KeyPoint> kp_object;
	//store <<"activeKeypoints" << activeKeypoints;

	//cv::write(store,"activeKeypoints",activeKeypoints);
	//cv::write(store,"trackedKeypoints",trackedKeypoints);
	std::vector<cv::Point2f> vPoints =std::vector<cv::Point2f>() ;

	vPoints.push_back(centerToTopLeft);
	vPoints.push_back(centerToTopRight);
	vPoints.push_back(centerToBottomLeft);
	vPoints.push_back(centerToBottomRight);

	write(store,"center",vPoints);
	...
	ALOG("En load");
	  ...
	  descriptorMatcher = cv::DescriptorMatcher::create(matcherType.c_str());
	   ...
	   // featuresDatabase = cv::Mat();
		...
		store["detectorType"] >> detectorType ;
		store["featuresDatabase"] >> featuresDatabase;

		std::vector<cv::KeyPoint> kp_object;
		std::vector<int> kp_object_int;

		cv::FileNode kptFileNode = store["activeKeypoints"];
		read( kptFileNode, kp_object );

		store["activeKeypoints_int"] >> kp_object_int;

		ALOG("load activeKeypoints_int %d",kp_object_int.size());
		ALOG("load activeKeypoints %d",kp_object.size());

		for (int i=0;i<kp_object.size();i++)
		{

			activeKeypoints.push_back(std::make_pair(kp_object[i],kp_object_int[i]));

		}
		//nbInitialKeypoints = activeKeypoints.size();

		store["classesDatabase"] >> classesDatabase;
		store["springs"] >> springs;
		store["selectedFeatures"] >> selectedFeatures;
		store["selectedClasses"] >> selectedClasses;
		store["votes"] >> votes;

		ALOG("load springs %d",springs.size());
		...
		outliers.clear();
		kptFileNode = store["outliers"];
		read( kptFileNode, kp_object );
		store["outliers_int"] >> kp_object_int;


		for (int i=0;i<kp_object_int.size();i++)
			{
			outliers.push_back(std::make_pair(kp_object[i],kp_object_int[i]));
		...
		kp_object.clear();
		kp_object_int.clear();
		kptFileNode = store["trackedKeypoints"];
		read( kptFileNode, kp_object );
		store["trackedKeypoints_int"] >> kp_object_int;

		ALOG("load trackedKeypoints_int %d",kp_object_int.size());


			for (int i=0;i<trackedKeypoints.size();i++)
				{
				  trackedKeypoints.push_back(std::make_pair(kp_object[i],kp_object_int[i]));

				}

			char s[20];
		int sqrcount= store["squareFormCount"];

		nbInitialKeypoints = (int) store["nbInitialKeypoints"];

		ALOG("squareFormCount %d",sqrcount);

		std::vector<float> fl = std::vector<float>();
		...
		{	fl.clear();
			...
			kptFileNode = store[s];
			read( kptFileNode, fl );
			squareForm.push_back(fl);
			ALOG("vector %d",fl.size());

		}


		int anglescount=store["anglesCount"];
		for (int i=0;i<anglescount;i++)
				{	fl.clear();
					...
					read( kptFileNode, fl );
					angles.push_back(fl);
					ALOG("vector angles %d",fl.size());
				}

		std::vector<cv::Point2f> vPoints =std::vector<cv::Point2f>() ;

		kptFileNode = store["center"];
		read( kptFileNode, vPoints );

		centerToTopLeft = vPoints[0];
		centerToTopRight = vPoints[1];
		centerToBottomLeft = vPoints[2];
		centerToBottomRight= vPoints[3];
